...
import { buildPlayerShip } from "./ship.js?v=20260603-cannon-line-v6";
import { DamageControlSystem } from "./damage-control.js?v=20260603-fort-silent-v1";
import {
...
} from "./collision-profile.js?v=20260602-default-profile-v2";
...
function selectItem(key) {
  selectedKey = sourceEntries.has(key) || overrideEntries.has(key) ? key : null;
...
renderer.domElement.addEventListener("pointerdown", (event) => {
  ...
  const hit = pickRay.intersectObjects([...overrideSelectables, ...sourceSelectables], false)[0];
...
$("import-json").addEventListener("click", () => {
  try {
    config = normalizeCollisionProfile(JSON.parse(json.value));
    rebuildLiveCollision(null);
    resetWalk();
  } catch (error) {

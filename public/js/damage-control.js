...
import { DEFAULT_HOLD_COLLISION } from "./collision-profile.js?v=20260602-default-profile-v2";
...
function holdWaterMaterial(oceanMaterial) {
  const sourceNormal = oceanMaterial?.uniforms?.normalSampler?.value;
  const normalMap = sourceNormal?.clone?.() || null;
  if (normalMap) {
    normalMap.wrapS = normalMap.wrapT = THREE.RepeatWrapping;
    normalMap.repeat.set(2.2, 8.5);
  }
  ...
  if (normalMap) material.normalScale.set(0.82, 0.82);
...
export class DamageControlSystem {
  ...
  _buildHold() {
    ...
    const holdWater = holdWaterMaterial(this.waterMaterial);
    this.holdWaterNormalMap = holdWater.normalMap;
    this.water = new THREE.Mesh(
      new THREE.PlaneGeometry(this.holdWidth - 0.7, this.holdDepth - 0.7),
      holdWater.material
    );
    this.water.name = "HoldWater";
    this.water.rotation.x = -Math.PI / 2;
    this.water.position.set(this.holdCenterX, this.floorY + 0.06, this.holdCenterZ);
  ...
  _scatterPlanks() {
    ...
    for (let pile = 0; pile < BOARD_PILE_COUNT; pile++) {
      ...
      pallet.position.set(x, this.floorY + 0.11, z);
  ...
  takePlank(rig, camera) {
    const position = rig?.position;
    if (!position || !this._canCollectWater(position) || this.heldPlank) return false;
  ...
  scoopWater(rig, camera) {
    const position = rig?.position;
    if (!position || !this._canCollectWater(position) || this.waterLevel < 1 || this.bucketFull) return false;
  ...
  getPrompt(rig) {
    ...
    const insideHold = this._insideHold(position);
    const canCollectWater = this._canCollectWater(position);
    if (insideHold || canCollectWater) {
      const breach = this._nearestActiveBreach(position);
      if (insideHold && breach && this.heldPlank) return "E - заколотить пробоину доской";
      const plank = this._nearestPlank(position);
      if (canCollectWater && plank && !this.heldPlank) return "E - взять доску для ремонта";
      if (canCollectWater && this.waterLevel >= 1 && !this.bucketFull) return "E - зачерпнуть воду ведром";
      return "";
    }
  ...
  interact(rig, camera) {
    ...
    if (insideHold || canCollectWater) {
      const breach = this._nearestActiveBreach(position);
      if (insideHold && breach && this.heldPlank) {
        this.heldPlank.removeFromParent();
        this.heldPlank = null;
        this._patchBreach(breach);
        this.onMessage("Пробоина заколочена.");
      ...
      const plank = this._nearestPlank(position);
      if (canCollectWater && plank && !this.heldPlank) {
        this._setHeldPlank(camera, plank);
        this.onMessage("Доска взята. Подойди к пробоине и нажми E.");
        return true;
      }

      if (canCollectWater && this.waterLevel >= 1 && !this.bucketFull) {
        this.waterLevel = Math.max(0, this.waterLevel - BUCKET_AMOUNT);
        this._setBucket(camera, true);
        this.onMessage("Ведро наполнено. Вынеси его на палубу и вылей за борт.");
        return true;
  ...
  update(dt) {
    ...
    this.waterLevel = Math.min(100, this.waterLevel + active.length * (1 + active.length * 0.16) * FLOOD_PER_HOLE * this.floodMultiplier * dt);
    ...
    if (this.holdWaterNormalMap) {
      this.holdWaterNormalMap.offset.x += dt * 0.025;
      this.holdWaterNormalMap.offset.y -= dt * 0.045;
    }
